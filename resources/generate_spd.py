...
if __name__ == "__main__":
    ...
    print("Eigenvalues (linear):", np.linalg.eigvalsh(A1)[:5], "…", np.linalg.eigvalsh(A1)[-5:])

    # 2) Log-uniform spacing, κ=1e6
    A2 = make_spd_matrix(50, condition_number=1e6, distribution="log", random_state=0)
    print("Eigenvalues (log):", np.linalg.eigvalsh(A2)[:5], "…", np.linalg.eigvalsh(A2)[-5:])
    ...
    print("Eigenvalues (clustered):", np.linalg.eigvalsh(A3)[:5], "…", np.linalg.eigvalsh(A3)[-5:])
